...
import { extToMime, checkFileExistsSync } from './common';
...
async function queuePrivateFile(filePath: string, login: string, driveId: string): Promise<any> {
	...
	if (extension !== '.enc' && stats.size !== 0 && !fileName.startsWith('~$')) {
		...
		const newFileToQueue: ArFSLocalPrivateFile = ArFSLocalPrivateFile.From({
			...
			txId: '0',
			dataTxId: '0',
			dataSyncStatus: 1, // Sync status of 1 requires a data tx
			syncStatus: 1 // Sync status of 1 requires a metadata tx
		});
...
async function queuePrivateFolder(
	...
	if (isQueuedOrCompleted || fileName === 'New folder') {
		...
		const folderToQueue: ArFSLocalPrivateFile = ArFSLocalPrivateFile.From({
			...
			txId: '0',
			dataTxId: '0',
			dataSyncStatus: 0, // Folders do not require a data tx
			syncStatus: fileMetaDataSyncStatus // Sync status of 1 requries a metadata tx
		});
